...
import LogoResp from '../Components/LogoResp';
...
const toggleSidebar = () => {setIsOpen(!isOpen)};
    ...
    <div className="min-h-screen overflow-x-hidden" style={{maxWidth:"100vw"}}>
        ...
        {showingNavigationDropdown && <ShadeScreen toggleSidebar={()=>setShowingNavigationDropdown(false)}/>}
            <div className="mx-auto">
                <div className={`sticky top-0 flex justify-between h-16 bg-gris-100 ${
                showingNavigationDropdown ? " bg-gris-100" : ""
                }` } >
                    <div className="flex">
                    ...
                    <div className={`-mr-2 flex items-center sm:hidden bg-gris-100 ${showingNavigationDropdown ? " bg-gris-100" : ""}`}>
                        <button
                        onClick={() => setShowingNavigationDropdown((previousState) => !previousState)}
                        className={`bg-gris-100 inline-flex items-center justify-center p-2 pr-4 rounded-md text-gray-600  transition duration-150 ease-in-out ${
                            showingNavigationDropdown ? " bg-gris-100" : ""
                            }`}
                        >
           ...
           <div
            className={`transition-height shadow-xl 'bg-gris-100' ${
                showingNavigationDropdown ? 'transition-height-open bg-gris-100 ' : 'bg-gris-100'
            }`}
            >
                <div className="pb-3 space-y-1 ml-8 'bg-gris-100'">
